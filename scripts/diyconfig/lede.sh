...
setLedePlugins()
{
	...
	if [ ! -d "${plugins_path}/kiddin9" ]; then
		print_log "INFO" "custom config" "正在获取kiddin9资源，请等待..."
	...
	print_log "INFO" "custom config" "正在获取golang资源，请等待..."
...
setLedeThemes()
{
	...
	# 默认主题
	sed -i 's/luci-theme-bootstrap/luci-theme-argon/g' ${source_path}/feeds/luci/collections/luci/Makefile
}
...
setLedeConfig()
{
	...
	# 设置编译信息
	{
		file="${source_path}/package/lean/default-settings/files/zzz-default-settings"
		if [ -e ${file} ]; then
			print_log "INFO" "custom config" "设置编译信息，请等待..."
