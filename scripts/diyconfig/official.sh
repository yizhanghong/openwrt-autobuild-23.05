...
setOfficialPlugins()
{
	...
	if [ ! -d "${plugins_path}/shidahuilang" ]; then
		print_log "INFO" "custom config" "正在获取shidahuilang资源，请等待..."
	...
	if [ ! -d "${plugins_path}/sirpdboy-package" ]; then
		print_log "INFO" "custom config" "正在获取sirpdboy-package资源，请等待..."
...
setOfficialThemes()
{
	...
	print_log "INFO" "custom config" "正在获取luci-theme-argon资源，请等待..."
	...
	print_log "INFO" "custom config" "正在获取luci-theme-argon-config资源，请等待..."
	...
	# 默认编译选项
	{
		...
		if [ -e ${file} ]; then
			...
			if grep -q "+luci-light" ${file}; then
				print_log "INFO" "custom config" "修改默认主题，请等待..."
				sed -i 's/luci-light/luci-theme-argon/g' ${file}
			fi
			
			# 取消http编译
			print_log "INFO" "custom config" "修改uhttpd编译，请等待..."
		...
		if [ -d "${source_path}/feeds/luci/collections/luci-light" ]; then
			print_log "INFO" "custom config" "删除插件luci-light，请等待..."
		...
		if [ -e ${file} ]; then
			print_log "INFO" "custom config" "修改插件luci-ssl，请等待..."
			if grep -q "+luci-light" ${file}; then
		...
		if [ -e ${file} ]; then
			print_log "INFO" "custom config" "修改插件luci-ssl-openssl，请等待..."
			if grep -q "+luci-light" ${file}; then
				sed -i 's/+luci-light //g' ${file}
			fi
		fi
